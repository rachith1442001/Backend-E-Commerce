        ...
        for (let i = 0; i < rowDataAsStringArrays.length; i++) {
            ...
            if (row.includes('Company')) {
                ...
                if (i + 2 < rowDataAsStringArrays.length) {
                    ...
                    currentCompany.Ticker = nextRow[1] || ''; // Ticker
                    currentCompany.SecurityID = nextRow[2] || ''; // Security ID
                    currentCompany.MeetingType = nextRow[3] || ''; // Meeting Type
                    currentCompany.Contested = nextRow[4] || ''; // Contested
                    currentCompany.AgendaType = nextRow[5] || ''; // Agenda Type
                ...
                for (let j = i + 1; j < rowDataAsStringArrays.length; j++) {
                    ...
                    const ballotData = {
                        BallotId: nextRow[0] || '',
                        ProposalText: nextRow[1] || '',
                        ManagementRec: nextRow[4] || '',
                        Vote: nextRow[5] || ''
                    };
